-- Magic Link Email Validation Function
-- This function checks if an email exists in our profiles table before allowing magic link sending
...
-- Create a view for magic link validation stats
CREATE OR REPLACE VIEW magic_link_validation_stats AS
SELECT 
    COUNT(*) as total_validation_attempts,
    COUNT(CASE WHEN is_activated = true THEN 1 END) as activated_accounts,
    COUNT(CASE WHEN is_activated = false THEN 1 END) as unactivated_accounts
FROM (
    SELECT DISTINCT email, is_activated
    FROM profiles
    WHERE email IS NOT NULL
) email_stats;

-- Add RLS policies for the functions
CREATE POLICY "Users can validate their own email" ON profiles

...
const SecretaryAnnouncements = () => {
  ...
  const { data: announcements, isLoading, error } = useQuery(['secretary_announcements'], fetchAnnouncements);
  ...
  const addAnnouncementMutation = useMutation(async (newAnnouncement) => {
    ...
    onSuccess: () => { setShowAdd(false); queryClient.invalidateQueries(['secretary_announcements']); }
  });
